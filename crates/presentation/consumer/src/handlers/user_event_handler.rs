...
impl UserEventHandler {
    ...
    pub async fn handle(&self, event: UserEvent) {
        info!(
            "Received user event: type={}, user_id={}, email={}",
            event.event_type, event.user_id, event.email
        );

        let result = match event.event_type.as_str() {
            "user.created" => self.handle_user_created(&event).await,
            "user.updated" => self.handle_user_updated(&event).await,
            "user.deleted" => self.handle_user_deleted(&event).await,
            _ => {
                warn!(
                    "Unknown event type: {} for user: {}",
        ...
        match result {
            Ok(_) => {
                info!(
                    "Successfully processed {} event for user: {}",
                    event.event_type, event.user_id
                );
            }
            Err(e) => {
                error!(
                    ...
                    event.event_type, event.user_id, e
                );
            }
        }
    }
