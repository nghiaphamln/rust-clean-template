...
use rust_clean_consumer_lib::{UserEventHandler, models::UserEvent};
...
async fn main() -> std::io::Result<()> {
    ...
    let queue_name = env::var("RABBITMQ_QUEUE")
        .unwrap_or_else(|_| "rust_clean_queue".to_string());

    info!("Connecting to database...");
    let _database = Database::new(&database_url, pool_max).await
        .expect("Failed to connect to database");
    info!("Database connected");

    info!("Connecting to RabbitMQ...");
    let rabbitmq_connection = RabbitMQConnection::new(&rabbitmq_url).await
